...
function baseGrid(){return {left:46,right:24,top:42,bottom:38,containLabel:true};}
function axis(name){return {name:name||'',nameTextStyle:{color:'#9b9387',fontSize:10,padding:[0,0,0,0]},axisLine:{lineStyle:{color:'#d8d0c3'}},axisTick:{show:false},axisLabel:{color:'#8d867a',fontFamily:'Geist Mono',fontSize:10},splitLine:{lineStyle:{color:'#ebe4d8'}}};}
...
function legend(){return {top:8,right:12,itemWidth:9,itemHeight:9,textStyle:{color:'#716b61',fontFamily:'Geist',fontSize:11}}}
...
async function main(){
 ...
 mountedChart('weekdayChart',{tooltip:tooltip(),legend:legend(),grid:baseGrid(),xAxis:{type:'category',data:a.weekdays.map(w=>w.weekday),axisLabel:{color:'#8d867a'},axisTick:{show:false},axisLine:{lineStyle:{color:'#d8d0c3'}}},yAxis:[{...axis('TIR %'),min:0,max:100},{...axis('mg/dL'),position:'right',splitLine:{show:false}}],series:[barSeries('TIR %',a.weekdays.map(w=>w.tir_70_180),palette.green),lineSeries('Avg glucose',a.weekdays.map(w=>w.avg_glucose),palette.blue,1,false)]});
 const wd=a.workout_days||[]; mountedChart('trainingChart',{tooltip:tooltip(),legend:legend(),grid:baseGrid(),xAxis:{type:'category',data:wd.map(w=>w.local_date.slice(5)),axisLabel:{color:'#8d867a',fontSize:10},axisTick:{show:false},axisLine:{lineStyle:{color:'#d8d0c3'}}},yAxis:[axis('minutes'),{...axis('TIR %'),position:'right',min:0,max:100,splitLine:{show:false}}],series:[barSeries('Z3-5 min',wd.map(w=>w.z3_5_min),palette.red),barSeries('Zone 2 min',wd.map(w=>w.z2_min),palette.cyan),lineSeries('Same-day TIR',wd.map(w=>w.same_day_tir),palette.green,1,false)]});
